...
if ($action == "create")
{
	if (isset($save))
	{
		...
		if ($id == "")
		{
			@mysql_query("INSERT INTO ".$table['links1']." (id) VALUES ('')");
			$lastid = @mysql_fetch_array(@mysql_query("SELECT LAST_INSERT_ID() as id FROM ".$table['links1']));
			$id = $lastid['id'];
		}

		@mysql_query("UPDATE ".$table['links1']." SET 
		...
		if (isset($id))
		{
			...
			$result = @mysql_query("SELECT * FROM ".$table['links1']." WHERE id='$id'");
...
else
{
	if (isset($del))
	{
		@mysql_query("DELETE FROM ".$table['links1']." WHERE id='$del'");
		exit(header("Location: redirect.php?module=".$module));
	}
...
<?
	...
	$result = @mysql_query("SELECT * FROM ".$table['links1']." ORDER BY position, name, id DESC");
